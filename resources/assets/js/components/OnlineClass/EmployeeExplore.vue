				        		...
				        		<!-- End Of Comment -->
					      		...
					      		<div class="col-md-12 col-lg-12 col-sm-12 col-xs-12 p-t-5 pb-0">
					                ...
					                <div class="col-sm-10 col-xs-10 col-md-10 col-lg-11 p-l-10 pr-0 pb-0 p-t-5">
						      			<input :id="'input-comment-'+post.id" class="form-control form-control-sm input-comment" type="text" placeholder="Post your comment" @keyup.enter.exact="addComment(post.class.code, post.id, $event.target.value)">
						      		</div>
					        	</div>
...
<script>
	...
	import moment from 'moment'
 	...
 	export default {
 		scrollToTop: false,
	 	props: ['user', 'school_id', 'spaces_url'],
	    data() {
	        return {
	        	moment: moment,
	    ...
	    methods: {
	    	...
	    	getPosts: async function(class_code) {
        	 	...
        	 	await axios.get('/admin/online-post/api/get/class-posts')
                 	.then(response => {
			            ...
			            delete: {
			             	text: 'Delete', // text for button
			              	...
			              	action: function(event){
			                	...
			                	// $.alert('Confirmed!');
				                axios.post('/admin/online-post/'+post_id+'/comment/'+comment_id+'/delete', {
					        		class_code 	: 	class_code,
						            post_id   	: 	post_id,
			            ...
			            delete: {
			             	text: 'Delete', // text for button
			              	...
			              	action: function(event){
			                	$("body").addClass("loading");
			                	// $.alert('Confirmed!');
				                var route = 'online-post/' + post_id +'?request_code=' + class_code;
				                $.ajax({
				                  	url: route,
				                  	type: 'DELETE',
