			...
			<div v-for="(post, index) in classPosts">
				...
				<div class="box shadow" :id="'post'+post.id">
					...
					<div class="box-body with-border" style="padding: 20px 20px 0px 20px !important;">
						<!-- Delete Post -->
						<div class="dropdown" v-if="employee_id == post.teacher_id">
		                  <a href="#" class="pull-right dropdown-toggle-post" data-toggle="dropdown"><i class="fa fa-ellipsis-h fa-2x"></i></a>
		                  <ul class="dropdown-menu pull-right" style="top: 25px; right: -18px;">
		                    <li>
		                     	<a href="javascript:void(0)" @click="deletePost(post.class_code, post.id)">Delete Post</a>
		                    </li>
		                  </ul>
		                </div>
		                <!-- End Of Delete Post -->
						...
						<div class="row posted_by">
		              	...
		              	<!-- Quiz -->
		              	<h4 class="post_by text-center" style="font-size: 14px;" v-if="post.class_quiz">
	                    	<a v-if="post.class_quiz.quiz" v-bind:href="'online-class/quiz/' + post.class_quiz.id + '?class_code=' + post.class_code">
	                    		View Quiz
	                    	</a>
	                 	</h4>
		              	<!-- Post Files -->
					...
					<div class="box-footer class-status" style="border-bottom-left-radius: 15px; border-bottom-right-radius: 15px;">
						...
						<div @click="likePost(post.class.code, post.id)" class="col-md-6 col-lg-6 col-sm-6 col-xs-6 text-center btn-star" data-toggle="tooltip" data-placement="top" title="Like">
							<span v-if="post.likes">
				        		<a :id="'btnLike'+post.id" href="javascript:void(0)" :class="post.employee_likes.includes(employee_id) ? 'star-on' : 'star-off'">
			        					...
			        					<div class="col-md-12 col-lg-12 col-sm-12 col-xs-12 p-t-5 pb-0" v-for="(comment, commentIndex) in post.comments" :id="'postComment'+comment.id">
							                ...
							                <div class="col-sm-10 col-xs-10 col-md-10 col-lg-11 pl-0 pr-0 pb-0 p-t-5">
							                	<div class="user-comment pt-0 pb-0 m-l-10">
							                		<!-- Comment Options -->
							                		<!-- Delete Comment -->
							                		<a href="javascript:void(0)" @click="deleteComment(post.class_code, post.id, comment.id)" class="pull-right p-t-5 p-r-5" v-if="employee_id == post.teacher_id || employee.employee_id == comment.commentable.employee_id"><i class="fa fa-times"></i></a>
									                <!-- Comment By -->
...
<script>
 	...
 	export default {
 		scrollToTop: false,
	 	props: [ 'code', 'user', 'school_id', 'spaces_url'],
	    data() {
	        return {
	        	moment: moment,
	    ...
	    methods: {
	    	...
	    	getPosts: async function(class_code) {
        	 	...
        	 	await axios.get('/admin/online-post/api/get/class-posts?class_code='+this.classCode)
                 	.then(response => {
                     	this.classPosts = response.data.posts.data;
                     	if(response.data.posts.next_page_url){    
                         	this.nextPage    = response.data.posts.next_page_url + '&class_code=' + this.classCode;
                     	}
                     	else{
                         	this.nextPage    = response.data.posts.next_page_url;
                     	}
                     	if(response.data.posts.prev_page_url){    
                         	this.prevPage   = response.data.posts.prev_page_url + '&class_code=' + this.classCode;
                     	}
                     	else{
	        ...
	        likePost(class_code, post_id) {
	        	...
	        	axios.post('/admin/online-post/'+post_id+'/like', {
	        		class_code 	: 	class_code,
		            post_id   	: 	post_id
		        }).then(res => {
		        	if(res.data.title == 'Success' && res.data.error == false) {
		        		// Update Like Button Color
		        		$("#btnLike"+res.data.data.id).removeClass(res.data.data.employee_likes.includes(_this.employee_id) ? "star-off" : "star-on");
		        		$("#btnLike"+res.data.data.id).addClass(res.data.data.employee_likes.includes(_this.employee_id) ? "star-on" : "star-off");
		        	}
		        	else {
		        		new PNotify({
	                      	title: 'Error',
	        ...
	        addComment(post_id, content) {
	        	...
	        	if(!content || content == null || !content.trim().length)
	        	{
		        	...
		        	axios.post('/admin/online-post/'+post_id+'/comment', {
			            ...
			            delete: {
			             	text: 'Delete', // text for button
			              	...
			              	action: function(event){
			                	...
			                	// $.alert('Confirmed!');
				                axios.post('/admin/online-post/'+post_id+'/comment/'+comment_id+'/delete?class_code=' + class_code, {
					        		class_code 	: 	class_code,
						            post_id   	: 	post_id,
			            ...
			            delete: {
			             	text: 'Delete', // text for button
			              	...
			              	action: function(event){
			                	$("body").addClass("loading");
			                	// $.alert('Confirmed!');
				                var route = 'online-post/' + post_id +'?class_code=' + class_code;
				                $.ajax({
				                  	url: route,
				                  	type: 'DELETE',
				                  	...
				                  	error: function(result) {
				                      	...
				                      	new PNotify({
				                          	...
				                          	text: "There's been an error. Your post might not have been deleted.",
				                          	type: "warning"
				                      	});
				                     	 $("body").removeClass("loading");
				                  	}
	    ...
	    mounted() {
	    	...
	    	Echo.channel(_this.schoolID + '-store-post-channel').listen('.' + _this.schoolID  + 'StorePostEvent', function (response) {
				...
				if(response.data.class_code === _this.classCode) {
