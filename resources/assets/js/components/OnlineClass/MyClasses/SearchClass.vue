        ...
        <div class="col-md-12 col-lg-12 oc">
            ...
            <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12" style="padding-left: 0; padding-right: 0;">
                <div class="form-group">
                    <select class="form-control" id="video_status" v-model="ongoing" @change="filterClass()">
                        <option value="" selected="true">All Classes</option>
                        <option value="on-going">On-going Classes</option>
                    </select>
                </div>
            </div>
            <!-- Searchbar -->
            ...
            <div class="col-lg-4 col-md-4 col-sm-10 col-xs-10 float-right" style="padding: 0; margin: 0">
                <input v-model="searchbar" @keyup.enter="searchClass(searchbar)" type="text" class="form-control smo-searchbox" id="search" placeholder="Search class">
            </div>
        </div>
                                    ...
                                    <div class="box-body with-border">
                                        ...
                                        <div class="row">
                                            <div class="">
                                                ...
                                                <h6 class = "class-desc"> 
                                                    {{ my_class.subject ? my_class.subject.subject_code : 'Unknown Subject' }} - <span>{{my_class.code}}</span>
                                                </h6>

                                                <!-- Class Subject Title -->
                                                <a v-bind:href="'online-post?class_code=' + my_class.code">

                                                    <h4 class="class-header" v-if="my_class.subject"> 
                                                        {{ my_class.subject.subject_title ? my_class.subject.subject_title : '-' }}
                                                        {{ my_class.summer ? '(Summer)' : '' }}
                                                    </h4>

                                                    <h4 class="class-header" v-else> Unknown Subject </h4>
                                                </a>

                                                <!-- Class Teacher -->
                                                <h6 class = "class-desc" v-if="my_class.teacher">
                                                    {{ my_class.teacher.prefix }}. {{ my_class.teacher.firstname }} {{ my_class.teacher.lastname }}  
                                                </h6>
                                                <h6 class = "class-desc" v-else> Unknown Teacher </h6>

                                                <!-- Grade and Section -->
                                                <h6 class = "class-desc" v-if="my_class.section">
                                                    {{ my_class.section.name_level }} {{ my_class.section.track_code ? '| ' + my_class.section.track_code : '' }}
                                                </h6>

                                                <!-- Class Students -->
                                                <h6 class = "class-desc" v-if="my_class.total_student">
                                                    Students : {{ my_class.total_student }}
                                                </h6>
                                                <h6 class = "class-desc" v-else>
                                                    Students : 0
                                                </h6>   
                                                ...
                                                <h5 style="padding: 5px 15px 0px 30px; margin-top: 0px;  margin-bottom: 0px;" v-if="my_class.ongoing">
                                                    <span class="badge label-success smo-vc" id="video_conference">
                                                        <i class="fa fa-video-camera"></i> Class On-going 
                                                    </span>
                                                </h5>
...
<script>
    ...
    export default {
        ...
        // watch: {
        //     searchbar: _.debounce(function(searchbar) {
        //         this.isTyping = true;
        ...
        methods: {
            ...
            requestPage: async function(url) {
                this.isLoading   = true;
                await axios.get(url)
                    .then(response => {
                        ...
                        if(response.data.classes.next_page_url){    
                            this.nextPage    = response.data.classes.next_page_url+ '&video_status=' + this.ongoing + '&search=' + this.searchQuery;
                        }
                        else{
                            this.nextPage    = response.data.classes.next_page_url;
                        }
                        if(response.data.classes.prev_page_url){    
                            this.prevPage   = response.data.classes.prev_page_url+ '&video_status=' + this.ongoing + '&search=' + this.searchQuery;
                        }
                        else{
                        ...
                        // console.log(this.videoStatuses);
                    });
            ...
            searchClass: async function(searchQuery) {
                ...
                await axios.get('/admin/teacher-online-class/api/get/class?video_status='+this.ongoing+'&search=' + this.searchQuery)
                    .then(response => {
