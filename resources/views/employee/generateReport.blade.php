	...
	<script>
        ...
        var body = 	[
                    ...
                    [
			        	...
			        	'MDSI - {{ $employee->employee_id }}',
                        '{{ $employee->full_name }}',
                        '{{ ($employee->email ?? "-") }}',
                        '{{ $employee->position }}',
                        '-',
                        '{{ $employee->date_hired }}',
                        '{{ $employee->total_years_of_service }}',
                        '{{ $employee->birthday }}',
	    ...
	    doc.autoTable({
	        ...
	        theme: 'grid',
            columns: [
		        { header: "Employee No.", dataKey: "emp_no" },
		        { header: "Fullname", dataKey: "full_name" },
                { header: "Email", dataKey: "email" },
                { header: "Position", dataKey: "position" },
                { header: "Status", dataKey: "status" },
                { header: "Date Hired", dataKey: "date_hired" },
                { header: "Years of Service", dataKey: "years_service" },
                { header: "Birthday", dataKey: "birthday" }
		    
		    ],
		    columnStyles: {
		        emp_no: { columnWidth: 20 },
		        full_name: { columnWidth: 50},
                email: { columnWidth: 35},
                position: { columnWidth: 50},
                status: { columnWidth: 20},
                date_hired: { columnWidth: 25},
                years_service: { columnWidth: 40},
		    },
	        ...
	        didDrawPage: function (data) {
             ...
             var base64Img = "{{ $schoollogo }}";
	            ...
	            doc.setFontSize(8);
	            doc.setFontType("bold");
	            doc.myText("EMPLOYEE LIST", {align: "center"},0,40);
	    ...
	    doc.save('Employee List {{ Carbon\Carbon::now()->format('m-d-Y') }}.pdf');
	    history.go(-1);
	</script>
</body>
