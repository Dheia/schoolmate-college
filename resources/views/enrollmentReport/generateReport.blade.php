	...
	<script>
	    ...
	    var body = 	[
		    			@foreach($enrollments as $key => $enrollment)
			        		[
			        			...
			        			@endif
			        			'{{ $enrollment->residential_address }}',
			        			'{{ ucwords(strtolower($enrollment->emergency_contact_name_on_record)) }}',
			        			'{{ $enrollment->emergency_contact_number_on_record }}',
			        			'{{ $enrollment->gender }}'
			        		],
		        		@endforeach
	        		];
	    ...
	    doc.autoTable({
	        ...
	        columns: [
		        ...
		        { header: "Emerg Contact Person", dataKey: "emerg_person" },
		        { header: "Emerg Contact No.", dataKey: "emerg_number" },
		        { header: "Gender", dataKey: "gender" }
		    ],
		    columnStyles: {
		        number: { columnWidth: 8 },
		        address: { columnWidth: 30 },
