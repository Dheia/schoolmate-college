...
@section('content')
  ...
  <div class="row p-l-20 p-r-20">
    ...
    <div class="col-md-8 col-lg-8 col-one" style="border-radius: 5px;">
      ...
      <class-post-show-student :school_id="'{{ env('SCHOOL_ID') }}'" :code="{{ json_encode($class->code) }}" :user="{{ $user }}" 
        :user_id="{{ $user->id }}" :submitted_quiz="{!! json_encode($studentSubmittedQuiz) !!}"
        :spaces_url="'{{ env('DO_SUBDOMAIN') }}'" :post_id="{{$post->id}}">
      </class-post-show-student>
