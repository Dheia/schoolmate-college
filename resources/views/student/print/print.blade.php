	...
	<body>
        ...
        <main>
			<div class="col-12 m-0 p-0" style="margin-bottom: 0px !important; padding-bottom: 0px !important;">
							...
							<tbody>
								...
								<tr>
									...
									@if($student->schoolYear)
										<th>{{ $student->schoolYear->schoolYear }}</th>
									@else 
										<td></td>
									@endif
								...
								<tr>
									<td>Department:</td>
									<th class="text-capitalize">{{ $student->department->name }}</th>
								</tr>
								<tr>
									<td>Level:</td>
									@if(!$student->current_level ?? '-')
										<th class="text-capitalize">{{  $student->current_level }}</th>
									@else
										<th class="text-capitalize">{{  $student->level_applied_for }}</th>
									@endif
								</tr>
								@if($student->track !== null)
									<tr>
										<td>Track</td>
										<th>{{ $student->track->code }}</th>
									</tr>
								@endif
							</tbody>
						</table>
				...
				{{-- STUDENT INFORMATION }} --}}
				@include('student.print.student_info')

				{{-- PERSONAL INFORMATION }} --}}
				@include('student.print.student_personal_information')

				{{-- FAMILY BACKGROUND }} --}}
				@include('student.print.family_background')
				
				{{-- EMERGENCY CONTACT INFORMATION }} --}}
				@include('student.print.emergency_contact_info')
						...
						<table class="table" style="padding-bottom: 0px !important; margin-bottom: 0px !important;">
							<thead>
                                ...
                                <tr style="position: relative;">
                                    <td class="mt-4">
                                    	<b>Referral Type:</b> {{ strtoupper(strtolower($student->referredBy->medium)) }}
                                    </td>
                                </tr>
                                <tr style="position: relative;">
                                    <td>
                                    	<b>Referred by:</b> {{ strtoupper(strtolower($student->referredBy->referred_by)) }}
                                    </td>
                                </tr>
                                @endif
							</thead>
