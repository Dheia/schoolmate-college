...
@php
    ...
    $options = \App\Models\SchoolYear::all();
...
<div class="input-group" style="width: 100%">
    <span class="input-group-addon" id="basic-addon1" style="width: auto;  background: #d2d6de;">School Year</span>
    <select name="schoolYear_id" id="schoolYear" class="form-control" style="width: 100%; display: unset;">
        @foreach($options as $option)
            @if(isset($_GET['schoolYear_id']))
                <option value="{{ $option->id }}"  {{ $_GET['schoolYear_id'] == $option->id ? 'selected=true' : null }}>{{ $option->schoolYear }}</option>
            @else
                <option value="{{ $option->id }}" {{ $option->isActive ? 'selected=true' : null }}>{{ $option->schoolYear }}</option>
            @endif
        @endforeach
    </select>
</div>
