        ...
        <table 
            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                ...
                <tr post-render ng-repeat="item in items" class="array-row" >
                    ...
                    {{-- <div ng-model="totalRowAmount" ng-init="<% item.amount %>"></div> --}}
                    <span ng-init="sum = item.amount * item.quantity"></span>
                </tr>
                <tr class="array-row">
                    <td></td>
                    <td></td>
                    ...
                    <td>
                        <div class="form-group col-xs-12">
                            <input ng-show="isNumber(getTotal2)" type="text" class="form-control ng-pristine ng-untouched" value="<% getTotal2  %>" readonly="true">
                        </div>
                    </td>
                    <td></td>
                    <td></td>
            ...
            window.angularApp.controller('tableController', function($scope) {
                ...
                $scope.getTotal = function(){
                    ...
                    for(var i = 0; i < $scope.items.length; i++){
                        var item = $scope.items[i].amount * $scope.items[i].quantity;
                    }
                    return total;
                }
                ...
                $scope.$watch('items', function(a, b){
                    ...
                    if( typeof $scope.items != 'undefined' && $scope.items.length ){
                        ...
                        $scope.getTotal = function() {
                            var total = 0;
                            for(var i = 0; i < $scope.items.length; i++){
                                total += ($scope.items[i].unit_price * $scope.items[i].quantity);
                            }
                            return total;
                        }
                        
                        $scope.$watch('getTotal2', function(a, b) {
                            $scope.getTotal2 = $scope.getTotal();
                        });
