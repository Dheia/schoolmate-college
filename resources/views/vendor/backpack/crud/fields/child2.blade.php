...
<div 
    ...
    ng-controller="Child2TableController" 
    ...
    <div class="array-container form-group">
            ...
            <div ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; max = {{$max}}; min = {{$min}}; maxErrorTitle = '{{trans('backpack::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('backpack::crud.table_max_reached', ['max' => $max])}}'">
                <div ui-sortable="sortableOptions" ng-model="items" >
                    <div post-render ng-repeat="item in items" style="clear: both;">
                        <hr>

                         @foreach ($field['columns'] as $column)
                            <div class="form-group @if(isset($column['size']))  col-md-{{ $column['size'] }} @endif">
                        ...
                        @endforeach
                        <button ng-hide="min > -1 && $index < min" class="btn btn-sm btn-default" type="button" ng-click="removeItem(item);"><span class="sr-only">delete item</span>Remove <i class="fa fa-trash" role="presentation" aria-hidden="true"></i></button>
                    </div>
                </div>
            </div>
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            ...
            window.angularApp.controller('Child2TableController', function($scope){
