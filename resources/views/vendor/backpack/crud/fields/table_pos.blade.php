            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                ...
                <tr ng-repeat="item in items" class="array-row">
                    ...
                    @foreach( $field['columns'] as $prop => $label)
                        ...
                        @endif
                        @if( $label == "Item")
                            <input class="form-control input-sm" type="text" ng-model="item.{{ $prop }}" disabled="True">
                        @endif --}}
                        {{ $prop}}
                    <td>
                        @if( $label == "Barcode")
                            <input class="form-control input-sm" type="text" ng-model="item.{{ $prop }}" ng-change="getItemFromBarcode()">
                        @endif
                        @if( $label == "Item Code")
                            <input class="form-control input-sm" type="text" ng-model="item.{{ $prop }}" readonly="True">
                        @endif
                        @if( $label == "Desc")
                            <input class="form-control input-sm" type="text" ng-model="item.{{ $prop }}" readonly="True">
                        @endif
                        ...
                        @endif
                        @if( $label == "Qty")
                            <input class="form-control input-sm" type="text" ng-model="item.{{ $prop }}" ng-change="getSubTotal()">
                        @endif
                        @if( $label == "Sub-Total")
                            <input class="form-control input-sm" type="text" ng-model="item.{{ $prop }}" readonly="True" >
                        @endif
        ...
        <script>
            ...
            window.angularApp.controller('tableController', function($scope){
                ...
                $scope.getSubTotal = function() {
                    var barcode = $scope.items;
                    console.log(barcode);
                }

                $scope.getItemFromBarcode = function() {
                    var barcode = $scope.items;
                    console.log(barcode);
                    // console.log("Getting data from barcode");
                }
