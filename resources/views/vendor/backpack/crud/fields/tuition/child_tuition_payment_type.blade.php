...
<div 
    ...
    ng-controller="MandatoryController" 
...
@if ($crud->checkIfFieldIsFirstOfItsType($field, $fields))
    ...
    @push('crud_fields_scripts')
        ...
        <script>
            ...
            window.angularApp.controller('MandatoryController', function($scope, $rootScope){
               // $scope.payment_scheme_total = '';
              
                var commitmentPayments = {!! json_encode(App\Models\CommitmentPayment::active()->get()) !!}
                console.log("cp ", commitmentPayments); 
                ...
                $scope.$watch('items', function(a, b){
                    ...
                    if( typeof $scope.items != 'undefined' && $scope.items.length ){
                        ...
                        {
                            angular.forEach(commitmentPayments, function (v, k) {
                                $scope['total_' + v.snake] = 0;
                            });

                            if("{{ $crud->route }}" !== "admin/enrollment") {
                                console.log($scope.items);
                                angular.forEach(commitmentPayments, function (v, k) {
                                    console.log('k ', k);
                                    console.log('v ', v);
                                    $scope.items[k].payment_type = v.id.toString();
                                });
                                // $scope.items[0].payment_type = "1";
                                // $scope.items[1].payment_type = "4";
                                // $scope.items[3].payment_type = "4";
                            }

                            angular.forEach($scope.items, function (val, key) {
                                
                                if(val.tuition_fees === undefined) { $scope.items[key]['tuition_fees'] = 0; }
                                if(val.discount     === undefined) { $scope.items[key]['discount']     = 0; }

                                $scope.items[key]['total'] = (val.tuition_fees - val.discount).toFixed(2);
                            });
                          
                        }
                            // payment_scheme_total);
                            // $scope.grand_total = $scope.payment_scheme_total
