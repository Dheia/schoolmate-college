    ...
    <div class="row">
        ...
        @foreach ($secondary_dependency['model']::orderBy('page_name')->get() as $connected_entity_entry)
            ...
            @if($category !== $connected_entity_entry->page_name)
              <div class="col-md-12">
                <br>
                <label for=""><b>{{ title_case(str_slug($connected_entity_entry->page_name, ' ')) }}</b></label>
              </div>
              @php 
                    ...
                    <input type="checkbox"
                        ...
                        @if( ( isset($field['value']) && is_array($field['value']) && (  in_array($connected_entity_entry->id, $field['value'][1]->pluck('id', 'id')->toArray()) || isset( $secondary_ids[$connected_entity_entry->id])) || ( old($secondary_dependency['name']) &&   in_array($connected_entity_entry->id, old($secondary_dependency['name'])) )))
                             ...
                             @if(isset( $secondary_ids[$connected_entity_entry->id]))
                              disabled = disabled
                             @endif
                        @endif > {{ title_case($connected_entity_entry->{$secondary_dependency['attribute']}) }}
                        {{-- {{ title_case(str_slug($connected_entity_entry->page_name, ' ')) }} --}}
                    </label>
                </div>
            </div>
