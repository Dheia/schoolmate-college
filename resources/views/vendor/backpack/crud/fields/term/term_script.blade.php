		...
		$(window).bind("load", function () {
    		...
    		setTimeout(function () {
					...
					function watchLevel () {
						...
						var level_id 		= level.find('option:selected').val();
						var track_id 		= track.find('option:selected').val();
						var term_id 		= term.find('option:selected').val();
						...
						$.each(departmentItems.levels, function (kDpt, lvlVal) {
							if(lvlVal.id == level_id)
							{
								...
								// terms
								if(lvlVal.terms.length > 0)
								{
									var termOptions  = '';
									$.each(lvlVal.terms, function (termKey, termVal) {
										// $('.track-wrapper').removeClass('hidden');
										termOptions += '<option value="' + termVal.id + '">' + termVal.name + '</option>';
