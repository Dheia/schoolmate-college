        ...
        <table class="table table-bordered table-striped m-b-0" ng-init="field = '#{{ $field['name'] }}'; items = {{ $items }}; max = {{$max}}; min = {{$min}}; maxErrorTitle = '{{trans('backpack::crud.table_cant_add', ['entity' => $item_name])}}'; maxErrorMessage = '{{trans('backpack::crud.table_max_reached', ['max' => $max])}}'; emptyRow='{{ $empty_row }}';">
            ...
            <tbody ui-sortable="sortableOptions" ng-model="items" class="table-striped">
                ...
                <tr ng-repeat="item in items" class="array-row">
                    ...
                    <td>
                        <input class="form-control input-sm" type="text" ng-model="item.{{ $prop }}" required="true">
                    </td>
                    @endforeach
            ...
            window.angularApp.controller('tableController', function($scope){
                ...
                $scope.$watch('items', function(a, b){
                    ...
                    for (i = 0; i < $scope.items.length; i++) {
                        choices += '<label class="radio-inline" for="answer_'+String.fromCharCode(letter.charCodeAt(0) + i)+'">'+
                            '<input type="radio" id="answer_'+String.fromCharCode(letter.charCodeAt(0) + i)+'" name="answer" value="'+String.fromCharCode(letter.charCodeAt(0) + i)+'" class="radio" required> '+String.fromCharCode(letter.charCodeAt(0) + i)+
                        '</label>';
                    }
                    $('#answer_choice').empty();
                    $('#answer_choice').append(choices);
                }, true);
