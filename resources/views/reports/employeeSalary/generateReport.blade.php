	...
	<script>
        ...
        var body = 	[
                    ...
                    [
                        ...
                        '{{ number_format($salary->admin_pay, 2, '.', ',') }}',
                        '{{ number_format($salary->other_pay, 2, '.', ',') }}',
                        '{{ $salary->description }}',
	    ...
	    doc.autoTable({
            ...
            columns: [
                ...
                { header: "Salary Type", dataKey: "type" },
                { header: "Salary", dataKey: "salary" },
                { header: "Admin Pay", dataKey: "pay" },
                { header: "Other Pay", dataKey: "other" },
                { header: "Description", dataKey: "desc" },
		    
		    ],
		    columnStyles: {
		        emp_no: { columnWidth: 20 },
		        full_name: { columnWidth: 30},
                type: { columnWidth: 35},
                salary: { columnWidth: 18},
                pay: { columnWidth: 17},
                other: { columnWidth: 17},
		    },
	        ...
	        didDrawPage: function (data) {
             ...
             var base64Img = "{{ $schoollogo }}";
	            ...
	            doc.setFontSize(8);
	            doc.setFontType("bold");
	            doc.myText("EMPLOYEE SALARY LIST", {align: "center"},0,40);
	    ...
	    doc.save('Employee Salary List {{ Carbon\Carbon::now()->format('m-d-Y') }}.pdf');
	    history.go(-1);
	</script>
</body>
