@php
  if(backpack_user())
  {
    ...
    if(auth()->user() && (starts_with(\Request::path(), 'student')))
    {
       $extends = 'backpack::layout_student';
    }
    else if(auth()->user() && (starts_with(\Request::path(), 'parent')))
    {
...
{{--   
@extends(backpack_user() && (starts_with(\Request::path(), config('backpack.base.route_prefix'))) ? 'backpack::layout' : 'backpack::layout_guest') --}}
{{-- show error using sidebar layout if looged in AND on an student page; otherwise use a blank page --}}
