...
<body>
	...
	<script>
        ...
        var body = 	[
					...
					@foreach($school_calendars as $key => $school_calendar)
                    [
						
			        	'{{ date("F j, Y", strtotime($school_calendar->start_at)) }}',
                        ' {{ date("F j, Y", strtotime($school_calendar->end_at)) }}',
                        '{{ $school_calendar->title }}',
                        '{{ $school_calendar->description }}',
	    ...
	    doc.autoTable({
	        ...
	        theme: 'grid',
            columns: [
		        { header: "Start", dataKey: "date" },
                { header: "End", dataKey: "date" },
	        ...
	        didDrawPage: function (data) {
             ...
             var base64Img = "{{ $schoollogo }}";
	            ...
	            doc.setFontSize(8);
	            doc.setFontType("bold");
	            doc.myText("SCHOOL CALENDAR LIST", {align: "center"},0,40);
	    ...
	    doc.save('School Calendar List {{ Carbon\Carbon::now()->format('m-d-Y') }}.pdf');
	    history.go(-1);
	</script>
</body>
