<!DOCTYPE html>	
<head>
	<title>{{Config::get('settings.schoolname')}} | Employee Attendance</title>
	...
	<script>
	    ...
	    var body = 	[
		    			@foreach($contents as $key => $content)
				        		...
				        		[
				        			...
				        			'{{ $content->date_format }} ({{ $content->week_day }})',
									'{{ $content->start_time_formatted}}',
									'{{ $content->end_time_formatted}}',
									'{{ $content->remarks}}',
									'{{ $content->duration ?? '-' }}',
	    ...
	    doc.autoTable({
	        ...
	        didDrawPage: function (data) {
	            ...
	            doc.setFontSize(8);
	            doc.setFontType("bold");
	            doc.myText("Employee Attendance", {align: "center"},0,40);
				...
				@if($date_input->date_period == 'today')
					doc.setFontType("bold");
					doc.text('{{ $date_input->date_from }}', data.settings.margin.left + 20, 65);
				@else
					doc.setFontType("bold");
					doc.text('{{ $date_input->date_from }} - {{ $date_input->date_to }}', data.settings.margin.left + 20, 65);
				@endif
	    ...
	    doc.save('{{$employee->full_name}} - Employee Attendance ({{ Carbon\Carbon::now()->format('m-d-Y') }}).pdf');
	    history.go(-1);
	</script>
</body>
