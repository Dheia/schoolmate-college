...
class EmployeeController extends Controller
{
    ...
    public function getEnrollment ($enrollment_id)
    {
        ...
        $additional_fees = AdditionalFee::where('enrollment_id', $_enrollment->id)->with('user')->get();
        ...
        $tuition_list = [
            ...
            'total_additional_fees'         => $additional_fees->sum('amount'),
            'tuition'                       => $tuition,
            'selected_other_programs'       => $selected_other_programs,
            'selected_other_services'       => $selected_other_services,
            'total_selected_other_program'  => $total_selected_other_program,
    ...
    public function savePayment (Request $request)
    {
        ...
        // IF HAS ADDITIONAL FEE
        if(count($request->additional_fees) > 0) {
            $additional_fees = collect($request->additional_fees)->map(function ($value, $key) use ($request, $todayDate) {
                return collect($value)
                        ->put('user_id', $request->user()->id)
                        ->put('enrollment_id', $request->enrollment_id)
                        ->put('payment_method_id', $request->payment_method_id)
                        ->put('payment_historable_type', "App\AdditionalFee")
