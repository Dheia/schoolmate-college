...
// Notification
use App\Notifications\OnlineClassStartedNotification;
...
class StartOnlineClassJob implements ShouldQueue
{
    ...
    public function handle()
    {
        if($this->class->activeStudentSectionAssignment) {
            ...
            if(count(json_decode($studentnumbers))>0) {
                ...
                Notification::sendNow($studentCredentials, new OnlineClassStartedNotification($this->zoomMeeting, $this->class));
                ...
                $message    = 'Your ' . $class_name . ' class has started';
                $link       = $this->zoomMeeting->join_url;
